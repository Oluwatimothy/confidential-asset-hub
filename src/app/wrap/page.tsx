...
function PairSelector({ pairs, selected, onSelect }: {
  ...
  return (
    <div className="grid gap-2">
      {pairs.map((pair, i) => (
        <button key={i} onClick={() => onSelect(pair)} className={`flex items-center gap-3 rounded-xl border p-3 text-left transition-all ${selected?.token.address === pair.token.address ? 'border-amber-400/50 bg-amber-400/5' : 'border-zinc-800 hover:border-zinc-700 hover:bg-zinc-900'}`}>
          ...
          {selected?.token.address === pair.token.address && <CheckCircle2 className="h-4 w-4 text-amber-400 shrink-0 ml-auto" />}
        </button>
      ))}
    </div>
  );
    ...
    return (
      <Card>
        <CardContent className="p-6 space-y-3">
          ...
          <Button variant="outline" className="w-full" onClick={() => { shield.reset(); approve.reset(); setAmount(''); }}>
            Wrap more
          </Button>
        </CardContent>
      </Card>
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base">Wrap {pair.token.symbol} to {pair.confidentialToken.symbol}</CardTitle>
        <CardDescription>
          {allowance !== undefined && (
            <span>Approved: {formatTokenAmount(allowance, pair.token.decimals)} {pair.token.symbol}</span>
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 pt-0">
        ...
        <div className="rounded-lg bg-zinc-800/50 p-3 text-xs space-y-1">
          <div className="flex justify-between text-zinc-400">
            ...
            <span className="text-amber-400">
              {amount && parseFloat(amount) > 0 ? `${amount} ${pair.confidentialToken.symbol} (encrypted)` : '—'}
            </span>
          </div>
          <div className="flex justify-between text-zinc-400">
            <span>Rate</span>
        ...
        <Button className="w-full" onClick={handleAction} disabled={!amount || isLoading} isLoading={isLoading}>
          {approve.isPending ? 'Approving…' : shield.isPending ? 'Wrapping…' : needsApproval ? `Approve ${pair.token.symbol}` : `Wrap ${pair.token.symbol}`}
        </Button>
        ...
        {error && (
          <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-3 flex items-start gap-2">
            ...
            <div>
              ...
              <button onClick={() => { shield.reset(); approve.reset(); }} className="mt-1 text-xs text-amber-400 hover:underline">Reset</button>
            </div>
          </div>
        )}
      </CardContent>
...
function WrapPageInner() {
  ...
  useEffect(() => {
    if (presetToken && validPairs.length > 0 && !selectedPair) {
      const found = validPairs.find((p) => p.token.address.toLowerCase() === presetToken.toLowerCase());
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-1">Convert public ERC20 tokens into encrypted ERC7984 confidential tokens.</p>
      </div>
      <Card>
        <CardHeader className="pb-3">
        ...
        <CardContent className="pt-0">
          {validPairs.length === 0
            ...
            : <PairSelector pairs={validPairs} selected={selectedPair} onSelect={(p) => setSelectedPair(p)} />
          }
        </CardContent>
      </Card>
